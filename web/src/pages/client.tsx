...
export function ClientOverviewPage() {
  ...
  const [metrics, setMetrics] = useState({
    ...
    suspiciousReports: '389',
    confirmedBlocked: '203',
    totalNum: 14892,
    smishingNum: 1247,
    suspiciousNum: 389,
  });
  ...
  useEffect(() => {
    getAnalyticsSummary()
      .then((data) => {
        const total = data.totalMessages || 15420;
        const smishing = data.classificationsByLabel?.smishing || 1389;
        const suspicious = data.alertsByStatus?.PENDING || 42;
        setMetrics({
          reportsReceived: total.toLocaleString(),
          likelySmishing: smishing.toLocaleString(),
          suspiciousReports: suspicious.toLocaleString(),
          confirmedBlocked: '203',
          totalNum: total,
          smishingNum: smishing,
          suspiciousNum: suspicious,
        });
      })
      .catch(() => {
    ...
    <ClientShell title="Executive Overview">
      ...
      <div className="stat-grid">
        ...
        <StatCard
          ...
          value={metrics.suspiciousReports}
        ...
        <StatCard
          ...
          value={metrics.confirmedBlocked}
      ...
      >
        ...
        >
          ...
          <DonutChart
            totalMessages={metrics.totalNum}
            smishingCount={metrics.smishingNum}
            suspiciousCount={metrics.suspiciousNum}
          />
        </div>
...
export function ClientMessagesPage() {
      ...
      .then((alerts) => {
        if (Array.isArray(alerts) && alerts.length > 0) {
          const mapped = alerts.map((alert) => ({
            id: alert.id.startsWith('#') ? alert.id : `#MSG-${alert.id.slice(0, 4)}`,
            sender: alert.sender,
            preview: alert.body,
            campaign: alert.bucket || 'Threat Telemetry Cluster',
            confidence: alert.score ? `${Math.round(alert.score * 100)}%` : '90%',
            score: alert.score ? Math.round(alert.score * 100) : 90,
            status: alert.label === 'smishing' ? 'Smishing' : 'Suspicious',
            timestamp: new Date(alert.receivedAt || alert.createdAt).toLocaleTimeString(
              'en-US',
              { hour: '2-digit', minute: '2-digit' },
            ),
            indicators: ['Mobile Telemetry', 'Live Alert'],
            carrier: 'Mobile Telemetry',
          }));
          setMessagesList(mapped);
        }
      })
      .catch(() => {
        // Fallback to static rawMessages
      });
  }, []);
  ...
  const activeMessages = messagesList.length > 0 ? messagesList : rawMessages;
