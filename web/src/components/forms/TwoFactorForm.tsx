...
export const TwoFactorForm: React.FC<TwoFactorFormProps> = ({ admin = false, email }) => {
  ...
  const targetEmail = email || (admin ? 'admin@bantai.research' : 'analyst@globe.com.ph');
  ...
  return (
    <form className="auth-form" onSubmit={handleSubmit} style={{ textAlign: 'center' }}>
      ...
      {resendNotice && (
        <div style={{ padding: '8px 12px', background: 'var(--green-bg)', border: '1px solid var(--green-border)', color: 'var(--green-text)', borderRadius: 8, fontSize: '0.75rem', marginBottom: 12 }}>
          {resendNotice}
        </div>
      )}
      ...
      <div style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', marginBottom: 20 }}>
        Code expires in{' '}
        <strong style={{ color: isExpired ? 'var(--red-text)' : 'var(--amber-text)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 16, fontSize: '0.8125rem' }}>
        <button
          type="button"
        ...
        <button
          type="button"
          onClick={() => navigate(admin ? ROUTES.ADMIN_LOGIN : ROUTES.LOGIN)}
