...
- [-] **5** Sprint 5: Integration, Hardening & Stretch Features
  ...
  - [x] **5.2** Design: technical specifications and interface designs for Sprint 5 items
    - [x] **5.2.1** Hardening plan (security + performance) *(Track A — Backend)*
...
- [-] **6** Sprint 6: Testing, Limited Deployment & Defense Readiness
  ...
  - [-] **6.4** Test: sprint-level unit and integration testing
    ...
    - [x] **6.4.6** False Positive/Negative confusion matrix on 20% held-out set *(Track B — AI/ML)* — early prep work by Maxene, done ahead of Sprint 6 and explicitly labeled "prep" in both commits, not the final deliverable. `scripts/create_holdout_set.py` carved a permanent 20% split (`ai/datasets/holdout/`, seed `20260618`, 3,236 rows: 1,785 Ham / 953 Spam / 498 Scam, `n_groups=2986` — grouped so near-duplicate messages don't leak across the split) out of the labeled pool, with the original backed up first. `scripts/evaluate_holdout.py` then scored the currently-deployed checkpoint against it: macro-F1 0.95 (Ham F1 0.964, Scam F1 0.962, Spam F1 0.925 — Spam's the weak spot, 113 false negatives vs 24 false positives, i.e. it under-flags Spam more than it over-flags). **That number is honestly caveated in the holdout manifest itself, not just here:** both the currently-deployed 2026-07-29 checkpoint and the 2026-08-17 retraining candidate were trained on the full pool *before* this split existed, so this run almost certainly evaluated the model against some of its own training data — not a clean never-seen test. The manifest says so explicitly and warns future runs off repeating the mistake. **The real 6.4.6 deliverable is the same script run against the first model trained after 2026-08-18** — that hasn't happened yet. Same PR also shipped three unrelated, genuinely-fixed items with their own value, logged in `DEV_LOG.md`: a real typosquat-detection bug (`str.lstrip("www.")` was stripping stray leading `w`/`.` characters past the intended prefix, corrupting domain checks like `www.wellsfargo.com`), a TOCTOU race in the retrain queue (concurrent `/retrain` calls could both pass the dedup check and double-enqueue), and two campaign-matching performance fixes. **Update 2026-08-27:** a pre-flight audit ahead of a planned final retrain found two more data-quality bugs the 08-26 candidate had trained under — 163 human review corrections never re-applied after the 08-26 dataset rebuild (fixed by wiring the correction step into `build_dataset.py` itself), and the correction script's hardcoded review-sheet filename list silently skipping 761 newly-reviewed rows (fixed via auto-discovery of any `review_sheet*.csv`). Dependency versions locked (`transformers==4.57.6`, `datasets==4.0.0`, `accelerate==1.14.0`, `huggingface-hub==0.36.2`) after an unpinned `transformers>=4.46` grabbed an untested v5 release mid-run and crashed. A third GPU retrain (Colab T4) on the fully corrected pool: gate says promote, macro-F1 0.9225 → 0.9461, 193 fixes vs 76 regressions, p≈0 (`ai/evaluation/retraining_run_2026-08-27.json`). This candidate — trained after the 2026-08-18 holdout freeze — got the first genuinely clean 6.4.6 evaluation: `scripts/evaluate_holdout.py` scored macro-F1 0.9592 on the 3,236-row holdout (Ham F1 0.976, Spam F1 0.952, Scam F1 0.949 but only 92.4% recall — 38/498 real scams missed, the honest weak point to lead with, not the headline macro-F1). Full record: `ai/evaluation/holdout_confusion_2026-08-27T10-32-15Z.json`. **Update 2026-09-12 — closes `[x]`:** the blocker above is resolved — the 08-27 holdout result (`ai/evaluation/holdout_confusion_2026-08-27T10-32-15Z.json`, commit `e0d1743`) is now on `develop` via merged PR #64 (`feature/ai-final-retrain-2026-08-27`), so the clean macro-F1 0.9592 holdout evaluation stands as the 6.4.6 deliverable.
