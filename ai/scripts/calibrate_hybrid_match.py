...
def rule_mask(scores: dict, hybrid_gate: float, lex_gate: float,
              domain_floor: float, threshold: float) -> np.ndarray:
...
def main(argv=None) -> int:
    ...
    print(f"Held-out members: {len(positives['cos'])}   "
          f"strangers: {len(negatives['cos'])}")
    print("=" * 74)
    ...
    print(f"{'domain':>12} {positives['dom'].mean():>10.4f} "
    ...
    for floor in DOMAIN_FLOOR_GRID:
        ...
        domain_rows.append({"floor": floor, "recall": round(r, 4),
                            "false_match_rate": round(f, 4)})
    ...
    print(f"{'emb gate':>9} {'lex gate':>9} {'recall':>9} {'false-match':>13} "
          f"{'vs baseline':>12}")
    rows = []
    for hg in HYBRID_GATE_GRID:
        for lg in LEXICAL_GATE_GRID:
            r = float(rule_mask(positives, hg, lg, 0.90,
                                DEFAULT_SIMILARITY_THRESHOLD).mean())
            f = float(rule_mask(negatives, hg, lg, 0.90,
                                DEFAULT_SIMILARITY_THRESHOLD).mean())
            rows.append({"hybrid_gate": hg, "lexical_gate": lg,
                         "recall": round(r, 4), "false_match_rate": round(f, 4)})
            print(f"{hg:>9} {lg:>9} {100*r:>8.1f}% {100*f:>12.1f}% "
                  f"{100*(r - base_recall):>+11.1f}pp")
