...
object BlockedNumbersApi {
    ...
    suspend fun list(token: String): Result<List<BlockedNumberEntry>> =
        get("/blocked-numbers", token).mapCatching { body -> parseList(JSONArray(body)) }

    /** POST /blocked-numbers — idempotent; safe to call for a number already blocked. */
    suspend fun block(token: String, sender: String): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
    ...
    suspend fun unblock(token: String, sender: String): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
    ...
    private fun post(path: String, body: JSONObject, token: String) {
        val connection = URL(ApiConfig.BASE_URL + path).openConnection() as HttpURLConnection
        try {
