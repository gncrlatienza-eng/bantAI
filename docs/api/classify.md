# Classification Service API

The ML inference service (FastAPI, Track B). Consumed **server-to-server** by the
NestJS backend — it is never called directly by the mobile app.

**Base URL (dev):** `http://localhost:8001`
**Auth:** optional `x-api-key`, off by default. Set `BANTAI_AI_SERVICE_API_KEY`
and `/classify`, `/summarize` and `/retrain` require that header (`401`
otherwise); `/health` and `/` stay open. Unset — the local default — those
routes accept any caller that can reach the port, and the service logs a
warning saying so at startup. Either way this service must not be exposed
publicly: keep it on the internal network with the backend as its only client.
The key is separate from `BANTAI_AI_BACKEND_API_KEY`, which is what this
service *sends* to the backend.
**Interactive docs:** `GET /docs` (auto-generated by FastAPI)
**Full ML pipeline docs:** [`../../ai/PIPELINE.md`](../../ai/PIPELINE.md) — how
the model behind this endpoint was built, trained, and evaluated.

---

## POST /classify

Classifies a single raw SMS body. The service masks structured identifiers
(emails, links, phone numbers, amounts, 4–8 digit codes), runs XLM-RoBERTa, and
returns both the predicted class and the routing decision derived from it.

### Request

```http
POST /classify
Content-Type: application/json
```

```json
{
  "message": "Your GCash account is locked. Verify now: http://gcash-verify.xyz/unlock"
}
```

| Field | Type | Required | Description |
|---|---|---|---|
| `message` | string | yes | Raw SMS body, unmodified. Must be non-empty. |

Send the body **exactly as received** — do not pre-mask or trim it. Masking is
part of this service so that training and inference see identical text.

### Response `200 OK`

```json
{
  "label": "Scam",
  "score": 0.9412,
  "scores": { "Ham": 0.0231, "Spam": 0.0357, "Scam": 0.9412 },
  "bucket": "blocked",
  "masked_text": "Your GCash account is locked. Verify now: <URL>",
  "campaign": {
    "cluster_id": "7",
    "similarity": 0.9991,
    "matched": true,
    "should_buffer": false,
    "lexical_similarity": 0.0,
    "match_reason": "embedding"
  },
  "indicators": [
    { "tag": "OTP / Account Phishing", "weight": 0.9 },
    { "tag": "Suspicious URL", "weight": 0.8 }
  ],
  "explanation_method": "keyword-fallback"
}
```

| Field | Type | Description |
|---|---|---|
| `label` | string | Predicted class: `Ham` · `Spam` · `Scam` |
| `score` | float 0–1 | Confidence of the predicted class |
| `scores` | object | Full softmax distribution over all three classes (sums to 1.0) |
| `bucket` | string | Routing decision: `safe` · `spam` · `blocked` · `unknown` |
| `masked_text` | string | The text actually fed to the model, with emails, links, phone numbers, amounts and 4–8 digit codes replaced by placeholders. **Reduced-risk, not anonymous** — names, addresses and 9+ digit identifiers (TINs, reference numbers) pass through, so store and log it as personal data (see `ai/preprocessing/masking.py`) |
| `campaign` | object \| null | Campaign-clustering result (Sprint 3). `null` when no centroids are loaded — see below. |
| `indicators` | array | Why the message was flagged: `{tag, weight}`, strongest first. May be empty. See below. |
| `explanation_method` | string | Which path produced `indicators`: `keyword-fallback` or `shap` |

### `indicators` and `explanation_method` (added 2026-09-20)

These are computed **inside this request**, so they always come from the fast
keyword tagger: `explanation_method` is `"keyword-fallback"` today, and `weight`
is capped at 0.9 on that path (`ai/service/indicator_tags.py`).

Real SHAP attribution is **not** computed here, so `"shap"` will not currently
appear. Measured on the deployed checkpoint (CPU, 2026-09-21/23): classification
takes ~0.06 s, while SHAP takes **~27 s median, up to ~90 s** on a long message
at the library's default sampling budget — far past the 3.5 s timeout the
backend applies to this endpoint (`backend/src/ai/ai.service.ts`). Running it
inline therefore made every message time out and fall back to the phone's
keyword heuristic (2026-09-20, fixed the next day; see
`tests/test_service.py::test_classify_never_runs_shap_inline`). A reduced
100-sample budget measures ~5 s and produced identical tags on six real scam
messages; that is the intended setting once SHAP runs as a background step
posting to the backend's `POST /sms/:messageId/indicators`. Until that exists,
treat `indicators` as keyword-derived.

**An empty array is valid** — a message can be flagged without matching any
known indicator pattern. Render the alert without a "why" section rather than
an empty one. Full reasoning: `docs/api/explainability.md`.

### `campaign` (Sprint 3, WBS 3.3.4; matching logic re-tuned Sprint 5, WBS 5.3.6)

| Field | Type | Description |
|---|---|---|
| `cluster_id` | string \| null | Matched `CampaignCluster` id, or `null` when nothing cleared any tier |
| `similarity` | float −1–1 | Cosine similarity to the closest active centroid |
| `matched` | bool | Whether the message cleared **any** of the three tiers below — not a single fixed threshold |
| `should_buffer` | bool | `true` when unmatched — hold the embedding for the next offline HDBSCAN pass |
| `lexical_similarity` | float 0–1 | Word-overlap (Dice coefficient) with the matched campaign's template. `0.0` when the campaign has no lexical profile or nothing matched |
| `match_reason` | string \| null | Which tier matched — `"domain"` \| `"hybrid"` \| `"embedding"`, see below. `null` when unmatched |

**The manuscript specifies a single 0.85 cosine threshold here.** Measured
against real data that attaches 54.5% of unrelated messages to a campaign
they don't belong to (`ai/PIPELINE.md` § "Stage 5b — measured limits"), so
the shipped system uses three tiers instead, checked in order of how much
evidence each carries — adviser-approved 2026-08-26:

| `match_reason` | Rule | Rationale |
|---|---|---|
| `domain` | shares a blasted domain **and** cosine ≥ 0.90 | link identity is near-conclusive on its own |
| `hybrid` | cosine ≥ 0.99 **and** `lexical_similarity` ≥ 0.45 | a coarse embedding filter wording then has to confirm |
| `embedding` | cosine ≥ **0.998** | the calibrated bar alone — no wording needed |

The embedding-only bar moves when the underlying model is retrained and
promoted (it was 0.999 under the checkpoint live through 2026-08-29, 0.998
since the 2026-08-30 promotion — see `ai/service/campaign.py:DEFAULT_SIMILARITY_THRESHOLD`
for the current value); the three-tier mechanism itself does not change.
Because the `embedding` tier is exactly the pre-hybrid rule, the tiers are
additive — the `domain` and `hybrid` tiers can only add matches the
old single-threshold system would have missed, never remove ones it caught.

The AI service **decides** the match; the backend **persists** it (writes
`clusterId` on the message, increments `messageCount`). The AI service has no
database access, which keeps the dependency one-directional.

`campaign` is `null` at cold start, before any clustering run has produced
centroids. This is an additive field — callers that ignore it are unaffected.
Full data flow: [`campaigns.md`](campaigns.md).

**Store `masked_text`, not the raw body,** wherever message content is persisted
for analytics or campaign clustering — URLs, phone numbers, amounts, emails and
OTPs are already replaced with `<URL>`, `<PHONE>`, `<AMOUNT>`, `<EMAIL>`, `<OTP>`.

### Why `label` and `bucket` are separate

`label` is **what the message is**; `bucket` is **what the app should do about
it**. They are deliberately different fields:

- `label` comes straight from the model and is what you record for metrics,
  confusion matrices and retraining.
- `bucket` folds in confidence thresholds, so a *low-confidence* `Scam` becomes
  `unknown` rather than `blocked`.

The backend should route on `bucket` and report on `label`. Never re-derive the
bucket yourself — threshold logic lives in this service so it can be tuned in one
place.

---

## Routing rules

Applied in order by `route()` in `ai/service/classifier.py`:

1. **argmax** — the highest-probability class wins.
2. **gap check** — if the winner leads the runner-up by less than
   `review_margin` (**0.15**), the model is torn (e.g. `0.50 / 0.50 / 0.00`) and
   the result is `unknown`, *regardless of thresholds*.
3. **threshold** — the winner must also clear its own confidence bar.

| Predicted `label` | Threshold to clear | Resulting `bucket` | Where the message goes |
|---|---|---|---|
| `Scam` | ≥ 0.90 | `blocked` | Hidden dropdown, retrievable |
| `Spam` | ≥ 0.60 | `spam` | Hidden dropdown, retrievable |
| `Ham` | ≥ 0.50 | `safe` | Inbox |
| any | fails gap **or** threshold | `unknown` | Inbox, flagged for user review |

`unknown` always stays in the inbox — a weak or ambiguous prediction must never
hide a message. The bar is highest for `blocked` because wrongly hiding a real
message is the most costly error.

### Worked examples

| `scores` (Ham / Spam / Scam) | Winner | Gap | Outcome |
|---|---|---|---|
| 0.02 / 0.04 / 0.94 | Scam 0.94 | 0.90 ✓ | `blocked` |
| 0.10 / 0.15 / 0.75 | Scam 0.75 | 0.60 ✓ | `unknown` — below the 0.90 Scam bar |
| 0.20 / 0.70 / 0.10 | Spam 0.70 | 0.50 ✓ | `spam` |
| 0.48 / 0.44 / 0.08 | Ham 0.48 | **0.04 ✗** | `unknown` — too close to call |

---

## GET /health

Liveness and readiness probe. Safe to poll.

```json
{ "status": "ok", "model_ready": true }
```

| Field | Type | Description |
|---|---|---|
| `status` | string | Always `ok` if the process is up |
| `model_ready` | bool | Whether fine-tuned weights are loaded and `/classify` will work |

The service starts and answers `/health` even with no model present, so
orchestration never blocks on training. Check `model_ready` before sending
traffic.

---

## Error responses

| Status | When | Backend should |
|---|---|---|
| `422 Unprocessable Entity` | `message` missing, empty, or longer than 4,000 characters | Fix the call — this is a client bug. The cap is ~2.5× a 10-segment concatenated SMS, and exists so an oversized body cannot be normalised, masked and tokenised before the 128-token truncation discards it |
| `401 Unauthorized` | `BANTAI_AI_SERVICE_API_KEY` is set and the `x-api-key` header is missing or wrong | Send the header; see Auth above |
| `503 Service Unavailable` | No fine-tuned model loaded | Retry with backoff; surface as "classification unavailable", do not treat as safe |
| `500 Internal Server Error` | Unexpected inference failure | Retry once, then log and queue for reprocessing |

A `503` means *unknown*, not *safe*. Never deliver a message as trusted because
classification failed.

---

## Configuration

Environment variables, prefix `BANTAI_AI_` (see `ai/.env.example`):

| Variable | Default | Purpose |
|---|---|---|
| `BANTAI_AI_MODEL_DIR` | `models/xlm-roberta-smishing` | Where fine-tuned weights are loaded from |
| `BANTAI_AI_MAX_LENGTH` | `128` | Max tokens per message (longer input is truncated) |
| `BANTAI_AI_SAFE_THRESHOLD` | `0.50` | Ham → `safe` |
| `BANTAI_AI_SPAM_THRESHOLD` | `0.60` | Spam → `spam` |
| `BANTAI_AI_BLOCK_THRESHOLD` | `0.90` | Scam → `blocked` |
| `BANTAI_AI_REVIEW_MARGIN` | `0.15` | Minimum top-two gap before any confident call |

Thresholds are tunable, not fixed constants — retune them once the trained
model's score distribution is observed on real traffic.

---

## Note on label vocabulary

Earlier drafts (and `docs/api/sms.md`) describe the classifier as returning
`Likely Smishing` · `Suspicious` · `Unknown`. **That is no longer the contract.**
The service returns a three-class `label` plus a separate four-value `bucket`,
which splits *what the message is* from *what to do about it* — the older scheme
conflated the two and had no way to express "unsolicited marketing".

Rough correspondence for anyone migrating:

| Old single value | Now |
|---|---|
| `Likely Smishing` | `label: "Scam"` + `bucket: "blocked"` |
| `Suspicious` | `label: "Scam"` or `"Spam"` below threshold → `bucket: "unknown"` |
| `Unknown` | `bucket: "unknown"` |
| *(no equivalent)* | `label: "Spam"` + `bucket: "spam"` |
| *(no equivalent)* | `label: "Ham"` + `bucket: "safe"` |

`docs/api/sms.md` was reconciled with the shipped code on 2026-07-30 and now
documents `Ham`/`Spam`/`Scam` correctly. The manuscript's own prose still uses
the old three-value wording — a known, separately-tracked mismatch to raise
with the adviser, not a code defect.
